...
(function() {
    CKEDITOR.plugins.add("uploadwidget", {
        lang: "cs,da,de,de-ch,el,en,eo,es,eu,fr,gl,hu,id,it,km,ko,ku,nb,nl,pl,pt,pt-br,ru,sv,tr,ug,uk,zh,zh-cn",
    ...
    CKEDITOR.tools.extend(CKEDITOR.fileTools, {
        addUploadWidget: function(a, d, e) {
            ...
            if (e.fileToElement) a.on("paste",
                function(f) {
                    ...
                    if (!f.dataValue && k)
                        for (h = 0; h < k; h++)
                            if (c = n.getFile(h), !e.supportedTypes || g.isTypeSupported(c, e.supportedTypes)) {
                                var l = e.fileToElement(c);
                                c = b.create(c);
                                l && (c[m](e.uploadUrl), CKEDITOR.fileTools.markElement(l, d, c.id), "loadAndUpload" != m && "upload" != m || CKEDITOR.fileTools.bindNotifications(a, c), f.dataValue += l.getOuterHtml())
                            }
                }, null, null, p);
            CKEDITOR.tools.extend(e, {
                ...
                init: function() {
                    var f = this,
                        d = this.wrapper.findOne("[data-cke-upload-id]").data("cke-upload-id"),
                        k = b.loaders[d],
                    ...
                    k.on("update", function(b) {
                        if (f.wrapper && f.wrapper.getParent()) {
                            a.fire("lockSnapshot");
                            b = "on" + e(k.status);
                            if ("function" !== typeof f[b] || !1 !== f[b](k)) h = "cke_upload_" + k.status, f.wrapper && h != c && (c && f.wrapper.removeClass(c), f.wrapper.addClass(h), c = h), "error" != k.status && "abort" != k.status || a.widgets.del(f);
                            a.fire("unlockSnapshot")
                        } else a.editable().find('[data-cke-upload-id\x3d"' +
                            d + '"]').count() || k.abort(), b.removeListener()
                    });
                    k.update()
                },
                replaceWith: function(b, d) {
                    if ("" === b.trim()) a.widgets.del(this);
                        ...
                        e ? (c.moveToBookmark(h), c.select()) : a.getSelection().selectBookmarks(l)
                    }
                }
            });
            a.widgets.add(d,
        ...
        bindNotifications: function(a, d) {
            ...
            d.on("update", function() {
                !b && d.uploadTotal && (e(), b = g.createTask({
