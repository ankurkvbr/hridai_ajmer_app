...
//        alert($(this).parent().parent().remove());
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (json.status == "success") {
                        $("#parent_" + img).remove();
                        if ($("#event-image-area .project_images").length < 0) {
                            $("#event-image-area").append('<div class="form-group" id="field-0">\n\
                                                                <div class="form-group file required" aria-required="true">\n\
                                                                <label>Project Image</label><input name="project_images[0][image]" class="image_field form-control" required="required" id="project-images-0-image" aria-required="true" type="file"></div>\n\
    ...
    $("#project").validate({
		...
		errorPlacement: function (error, element) {
				if(element.attr("name") == "project_description"){
				error.insertAfter($('.cke_editor_project_description'));
				}else
